...
def make_queries_prompt(queries_number, topic):
   return f"I'm exploring YouTube Shorts (vertical videos under 1 minute) related to different topics. To get a better understanding of what's popular, I want to search for a variety of specific queries related to a given topic. In the end of this text I will give you a topic, you should generate a list of {str(queries_number)} specific hashtags that will help me discover the most popular and relevant videos. These 20 tags together MUST cover all or almost all popular shorts which could potentially be created. Focus on how videos related to the topic might be titled, not just synonyms of the topic name. List the tags without numbering and without extra words. Separate them with a comma without any additional spaces etc. Don't use abstract names in square brackets, always use specific most popular examples instead of it. Your topic for now is \"{topic}\". Create hashtags in the language in which topic is given."
...
for tag in tags:
    for i in range(key_number):
        ...
        try:
            ...
            print(f"Successfully proceeded query {tag} using key", api_keys[i])
            ok = True
        except Exception as e:
            print(f"Error processing query {tag} using key {api_keys[i]}, error: {e}")
            continue
